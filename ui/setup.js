...
.ct-ed{position:fixed;inset:0;z-index:9999;display:grid;grid-template-rows:auto 1fr auto;
  background:linear-gradient(180deg,rgba(255,255,255,.92),rgba(255,255,255,.96));backdrop-filter:saturate(1.1) blur(8px)}
...
.ct-tool img{width:56px;height:56px;object-fit:cover;border-radius:10px;background:#f5f8ff;border:1px solid #edf1f7}
  ...
  const Picker={ el:null, bd:null, esc:null, onPick:null,
    ...
    create(){
      ...
      const bPhoto=btn('Foto','is-sm'), bCreate=btn('Anlegen','brand is-sm');
  ...
  const Editor={ el:null, draft:null, dirty:false, side:'RO', openPos:null,
    ...
    render(group){
      ...
      const draw=()=>{
        ...
        sideObj.slots.forEach(slot=>{
          ...
          if(this.openPos===slot.pos){
            ...
            const bPick=btn('Aus Tools wählen'), bNew=btn('Neues Werkzeug'), bDetach=btn('Entfernen','is-sm'), bCancel=btn('Abbrechen'), bSave=btn('Speichern','brand');
      ...
      function btn(label, cls=''){ const b=document.createElement('button'); b.className='btn '+cls; b.textContent=label; return b; }
  ...
  const UI={
    ...
    render(){
      ...
      const last=this.els.lastWrap; if(last){ last.innerHTML=''; list.slice(0,3).forEach(g=>{ const r=document.createElement('div'); r.className='kp'; r.style.display='flex'; r.style.gap='10px'; r.style.alignItems='center';
